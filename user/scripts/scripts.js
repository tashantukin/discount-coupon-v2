(function ()
{
  ...
  document.addEventListener("DOMContentLoaded", function ()
  {
        ...
        $.ajax({
          ...
          success: function (result)
          {
            ...
            if (discountDetails.result.length == 0) {
              ...
              totaldiscountcheckout =
                totaldiscountcheckout + parseFloat(coupondiscount);
              var couponspan =
                '<input type="hidden" class="coupon-msg" id="couponhidden"></span>';
    ...
    $(document).ready(function ()
    {
      ...
      if (url.indexOf("/user/manage/order/details") >= 0) {
        ...
        function sendEDM(invoiceID)
        {
          ...
          var data = {
            ...
            trackingNo: $("#trackingnumber").val(),
            trackingURL: $("#trackingurl").val(),
          };
      ...
      if (url.indexOf("/user/manage/orders") >= 0) {
        ...
        $("#export").on("click", function ()
        {
          if (!$(this).hasClass("disabled")) {
            $(this).attr(
              "download",
              file_name_start + "_" + file_name_end + "_Items.csv"
            );
            $("#link2").attr(
              "download",
        ...
        $("#link2").attr({
          href:
            packagePaths + "/downloads?file=invoice.csv&contentType=text/csv",
          target: "_blank",
        });
      ...
      if (url.indexOf("/user/order/ordersummary") >= 0) {
        if ($(".register-link").length) {
          ...
          var discountlabel =
            "<label class='lbl-coupon'>Discount Coupon</label>";
          ...
          var couponinput =
            "<p class='discount-total'>Discount Total <span class='pull-right'><span> - </span><span id='currencyCode'>" +
            mpCurrencycode +
            "</span>  <span id='currencySym'></span><span class='sub-total'><span id='price_amt'><span id='price_amt'></span> </span></p><div class='item-coupon-box'><div class='appand-coupon-rem'></div><div class='promocode-update'><form action=''><input type='text' name='coupon-code' id ='promocode' placeholder='PROMOCODE' class='pr-text' maxlength='10'><input type='text' name='coupon-code2' id ='promocode2' placeholder='PROMOCODE' class='pr-text' maxlength='10'><button type='button' class='apply-promo-btn disable' id='applycoupon'>Apply</button></form></div></div>";
                  ...
                  if ($(this).is(".withmerchantpromo")) {
                    ...
                    $(".pr_detail:not(.hasitempromo)", merchantbox).each(
                      function ()
                      {
                        ...
                        totalcostmerchant =
                          totalcostmerchant + parseFloat(total);
          ...
          var confirmModal =
            "<div class='popup-area cart-checkout-confirm' id ='plugin-popup'><div class='wrapper'> <div class='title-area'><h1>Please remove already expired coupons.</h1></div><div class='content-area'><span id ='main'><span id='addtext'> </span></div><div class='btn-area'>  <a  class='add-cart-btn' id='btn-ok'>OK</a></div></div></div>";
    ...
    function showPromoCodeV2()
    {
      ...
      var coupondiv =
        "<div class='item-coupon-box' discounttype='" +
        discounttype +
        "', discountval='" +
        discountVal +
        "', discountqty='" +
        ...
        "', couponleft'" +
        couponqty +
        "'><div class='appand-coupon'><div class='coupon-con'><span class='coupon-code' >PROMOCODE</span><span class='pull-right'> <span id='currencySym'></span><span class='sub-total'><span id='price_amt'></span> </span> <i title='Remove' class='fa fa-times remove-coupon'></i> </span></div></div></div>";
      if (merchants != null) {
        var notvalid = 0;
        var counters = 0;
          ...
          $(".mearchant_box").each(function ()
          {
            ...
            if (element == merchantid) {
              ...
              if (counter != 0) {
                ...
                $(".sub-total #price_amt:last", $(this)).text(
                  formatter.format(couponvalue)
                );
                $(".item-coupon-box:last", $(this)).addClass(
                  "withmerchantpromo"
                );
    ...
    function showItemCode()
    {
      ...
      $(".mearchant_box").each(function ()
      {
        ...
        $(".pr_detail:not(.hasitempromo)", $(this)).each(function ()
        {
          ...
          if (isLimited == 1) {
            //validate only if the coupon has Limited redemption
          ...
          if ($itemcoupon == itemguid) {
            ...
            var discounttag =
              "<span class='coupon-code' id ='discounttag'>Discount Applied</span>";
    ...
    function calculateTotal()
    {
      ...
      $(".mearchant_box").each(function ()
      {
        ...
        default_total <= totaldiscount
          ? (totaldiscount = default_total)
      ...
      // total discount
      $(".discount-total")
        .find(".sub-total #price_amt")
        .text(formatter.format(total_coupon_discount));
      //Total
      // subtotal_del2=  $('.l_box  p:contains("Sub Total") .sub-total').text();
      subtotal_del2 = $(".l_box .sub-total").first().text();
    ...
    function updateOrders_suntec()
    {
      ...
      $(".mearchant_box").each(function ()
      {
        ...
        $(".item-coupon-box", $(this)).each(function ()
        {
          ...
          $(this).attr("itemqty") != undefined
            ? (d_code = d_code + "-" + d_itemqty)
            : "";

          console.info("dcode " + d_code);

          var merchantdiscount = $(this)
            .find(".coupon-con .sub-total #price_amt")
            .text()
            .replace(/[^\d.-]/g, "");
          if (merchantdiscount == null || merchantdiscount == "") {
        ...
        var data = {
          order_guid: orderid,
          discounttype: d_type,
          discount_val: totalpercentage,
          coupon_code: JSON.stringify(couponlist),
          isLimited: d_limited,
          coupon_qty: d_qty,
          coupon_id: d_id,
    ...
    function showPromoCodeSpacetime()
    {
      ...
      var discount =
        '<div class="coupon-con" id="discount"><span class="pull-right"><span id="currencySym"></span><span class="sub-total"> <span id="price_amt" <span id="price_amt"></span></span> </span></span></div>';
      ...
      deliveryCharge = $(
        '.review-order-container:contains("Delivery cost") .review-order-price'
      ).text();
      currentTotal = $(
      ...
      couponvalue = parseFloat(
        calculatePercentage(discountVal, currentSubtotal1)
      );
    ...
    function showPromoCode()
    {
      ...
      $("#promodiv").prepend(promo);
      var discount =
        '<div class="coupon-con" id="discount"><span class="pull-right checkout-itm-tprice"><span id="currencySym"></span><span class="sub-total"><span id="price_amt" <span id="price_amt"></span></span> </span></span></div>';
      ...
      deliveryCharge = $(
        ".checkout-itm-total-sec .checkout-total-line1:nth-child(2) .checkout-itm-tprice"
      ).text();

      bulkdel = $(
        ".checkout-itm-total-sec .checkout-total-line1:nth-child(3) .checkout-itm-tprice"
      ).text();
    ...
    function showPromoCodeDel2()
    {
      ...
      $("#promodiv").prepend(promo);
      var discount =
        '<div class="coupon-con pull-right" id="discount"><span class="pull-right"><span id="currencySym"></span><span class="sub-total"><span id="price_amt" <span id="price_amt"></span></span> </span></span></div>';
      $("#couponinput").append(promocode);
    ...
    function returnError(errorType)
    {
      if (errorType == "Invalid") {
        ...
        errorType =
          '<span class="coupon-msg" id="msg">Coupon code expired.</span>';
      } else if (errorType == "Already exists") {
        errorType =
          '<span class="coupon-msg" id="msg">Coupon code exists.</span>';
    ...
    function discount_checkout()
    {
      waitForElement("#couponhidden", function ()
      {
        ...
        var promo =
          '<div class="checkout-totline-left" id="coupon_ordetails"></div> <div class="checkout-itm-tprice" id="couponvalue"></div> <div class="clearfix"></div>';
        last.append(promo);
        $("#coupon_ordetails").text("Discount Total");
        $("#couponvalue").text(
          "-" + mpCurrencycode + formatter.format(totaldiscountcheckout)
        );
      });
    }
